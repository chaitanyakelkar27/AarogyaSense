<script lang="ts">
	...
	async function loadCases() {
		try {
			...
			stats.pendingReview = allCases.filter(c => 
				c.status === 'PENDING' || c.status === 'UNDER_REVIEW' || c.status === 'FORWARDED_TO_CLINICIAN'
			).length;
			...
			<div class="max-w-7xl mx-auto px-4 py-4">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-4">
						...
						<div>
							<h1 class="text-xl font-bold text-gray-900">Clinician Portal</h1>
							<p class="text-sm text-gray-600">Medical case review and diagnosis</p>
						</div>
					</div>
					<div class="text-right">
						<p class="text-sm font-medium text-gray-900">{$authStore.user?.name}</p>
						<p class="text-xs text-gray-600">Clinician</p>
					</div>
				</div>
			</div>
		</header>
			...
			{:else}
				<div class="space-y-6">
					...
					<div class="grid grid-cols-4 gap-6">
						...
						<div class="bg-white rounded-lg shadow p-6">
							<p class="text-sm text-gray-600 mb-2">High Priority</p>
					...
					<div class="bg-white rounded-lg shadow">
						<div class="px-6 py-4 border-b border-gray-200">
							...
							<p class="text-sm text-gray-600 mt-1">Review and manage patient cases</p>
						</div>
