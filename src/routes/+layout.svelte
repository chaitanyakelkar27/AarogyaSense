	...
	const allNavItems = [
		{
			labelKey: 'nav.home',
			href: '/',
			roles: ['CHW', 'ASHA', 'ASHA_SUPERVISOR', 'CLINICIAN', 'DOCTOR', 'ADMIN']
		},
		{
			labelKey: 'nav.chwApp',
			href: '/chw',
		...
		{
			labelKey: 'nav.ashaPortal',
			href: '/asha',
			roles: ['ASHA', 'ASHA_SUPERVISOR', 'CLINICIAN', 'DOCTOR', 'ADMIN']
		},
		{ labelKey: 'nav.clinicianPortal', href: '/clinician', roles: ['CLINICIAN', 'DOCTOR', 'ADMIN'] }
	];

	// Filter nav items based on user role
	let navItems = $derived(() => {
		const userRole = $authStore.user?.role;
		if (!userRole) return [{ labelKey: 'nav.home', href: '/', roles: [] }];
...
<div class="flex min-h-screen flex-col">
	<header class="sticky top-0 z-40 w-full border-b border-border/40 bg-surface/80 backdrop-blur-md supports-[backdrop-filter]:bg-surface/60">
		<div class="flex h-16 w-full items-center justify-between px-4 sm:px-8">
			...
			>
				<svg
					class={`h-5 w-5 transition-transform ${menuOpen ? 'rotate-45' : ''}`}
					viewBox="0 0 24 24"
				>
					<path
					...
					{#if $authStore.isAuthenticated}
						<div class="rounded-lg bg-gray-50 p-4">
							...
							<p class="text-xs text-gray-600">
								{$authStore.user?.role} • {$authStore.user?.email}
							</p>
						</div>
						<button
							type="button"
	...
	<footer class="border-t border-border/60 bg-surface/80">
			...
			<div id="contact" class="space-y-4">
				...
				<p class="text-sm text-muted">
					Email <a
						...
						class="underline decoration-brand/40 underline-offset-4">care@aarogyasense.org</a
					>
		...
		<div class="border-t border-border/60 bg-surface/60">
			<div
				class="mx-auto flex max-w-6xl flex-col gap-2 px-6 py-6 text-xs text-muted md:flex-row md:items-center md:justify-between"
			>
