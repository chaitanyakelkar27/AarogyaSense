...
<div class="min-h-screen bg-background">
	{#if unauthorized}
		...
		<div class="flex min-h-screen items-center justify-center p-4">
			<div class="max-w-md w-full bg-surface rounded-2xl shadow-xl p-8 text-center">
				...
				<p class="text-surface-muted mb-6">You don't have permission to access the CHW Portal.</p>
				<div class="space-y-3">
					<p class="text-sm text-surface-muted">
						Your role: <strong>{$authStore.user?.role}</strong>
					</p>
					<a
						href="/"
	...
	{:else}
		<div class="w-full">
			<!-- Header -->
			<header
				class="bg-surface shadow-sm border-b border-border/50 sticky top-0 z-50 backdrop-blur-md bg-surface/90 mb-8"
			>
			...
			{#if activeTab === 'new-case'}
				...
				{#if saveMessage}
					<div
						class="mb-6 rounded-lg {saveMessage.startsWith('✅')
							? 'bg-green-50 text-green-800'
							: 'bg-red-50 text-red-800'} p-4 font-medium"
					>
						{saveMessage}
					</div>
				{/if}
				...
				<form class="space-y-6">
					...
					<div class="rounded-2xl bg-surface p-6 shadow-lg">
						...
						<div class="grid gap-4 md:grid-cols-2">
							...
							<div>
								<label for="age" class="mb-1 block text-sm font-medium text-surface-emphasis"
									>Age *</label
								>
								<input
									type="number"
							...
							<div>
								<label for="gender" class="mb-1 block text-sm font-medium text-surface-emphasis"
									>Gender</label
								>
								<select
									id="gender"
							...
							<div>
								<label for="village" class="mb-1 block text-sm font-medium text-surface-emphasis"
									>Village</label
								>
								<input
									type="text"
							...
							<div>
								<label for="emergency" class="mb-1 block text-sm font-medium text-surface-emphasis"
					...
					<div class="rounded-2xl bg-surface p-6 shadow-lg">
						...
						<div class="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
							{#each symptomOptions as symptom}
								<label
									class="flex items-center gap-2 cursor-pointer rounded-lg border border-border p-3 transition-colors hover:bg-surface-soft {selectedSymptoms.includes(
										symptom
									)
					...
					<div class="rounded-2xl bg-surface p-6 shadow-lg">
						...
						<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
							<div>
								<label for="temp" class="mb-1 block text-sm font-medium text-surface-emphasis"
									>Temperature (°C)</label
								>
								<input
									type="number"
							...
							<div>
								<label for="bp" class="mb-1 block text-sm font-medium text-surface-emphasis"
							...
							<div>
								<label for="hr" class="mb-1 block text-sm font-medium text-surface-emphasis"
							...
							<div>
								<label for="spo2" class="mb-1 block text-sm font-medium text-surface-emphasis"
							...
							<div>
								<label for="rr" class="mb-1 block text-sm font-medium text-surface-emphasis"
										...
										<div class="relative rounded-lg border border-border p-3">
											...
											{#if image.analysis}
												<div class="mt-2 rounded-lg bg-surface-soft p-3 text-sm">
													<p class="font-semibold text-surface-emphasis">
														{image.analysis.condition}
													</p>
													<p class="text-surface-muted">
														Confidence: {(image.analysis.confidence * 100).toFixed(1)}%
													</p>
							...
							{#if !capturedAudio}
								...
								{:else}
									<div class="rounded-lg bg-red-50 p-6 text-center">
										...
										<p class="mt-2 text-surface-muted">
											Recording... Please breathe or cough near the microphone
										</p>
								...
								<div class="rounded-lg border border-border p-4">
									...
									{#if capturedAudio.analysis}
										<div class="mt-3 rounded-lg bg-surface-soft p-3 text-sm">
											<p class="font-semibold text-surface-emphasis">
												{capturedAudio.analysis.condition}
											</p>
											<p class="text-surface-muted">
												Confidence: {(capturedAudio.analysis.confidence * 100).toFixed(1)}%
											</p>
											...
											{#if capturedAudio.analysis.audioFeatures.breathingRate}
												<p class="text-surface-muted">
													Breathing Rate: {capturedAudio.analysis.audioFeatures.breathingRate} breaths/min
												</p>
											{/if}
										</div>
									{/if}
					...
					{#if riskAssessment}
						<div class="rounded-2xl bg-surface p-6 shadow-lg">
							...
							<div class="space-y-4">
								<div
									class="flex items-center justify-between rounded-lg bg-gradient-to-r {riskAssessment.level ===
									'CRITICAL'
										...
										: riskAssessment.level === 'HIGH'
											...
											: riskAssessment.level === 'MEDIUM'
												...
												: 'from-green-500 to-green-600'} p-6 text-white"
								>
									<div>
				...
				<div class="space-y-4">
					{#if isLoadingCases}
						<div class="rounded-2xl bg-surface p-12 text-center shadow-lg">
							<div
								class="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-brand border-t-transparent"
							></div>
									...
									<div class="flex items-start justify-between">
										<div class="flex-1">
											<div class="flex items-center gap-3">
												...
												<span
													class="rounded-full border px-3 py-1 text-xs font-semibold {getStatusBadgeClass(
														caseItem.status
													)}"
												>
													{caseItem.status}
												...
												<span
													class="rounded-full px-2 py-1 text-xs font-semibold {getPriorityBadgeClass(
														caseItem.priority
													)}"
												>
											...
											{#if caseItem.status === 'APPROVED' || caseItem.status === 'REJECTED'}
												<div
													class="mt-3 rounded-lg {caseItem.status === 'APPROVED'
														? 'bg-green-50'
														: 'bg-red-50'} p-3"
												>
													<p
														class="text-sm font-semibold {caseItem.status === 'APPROVED'
															? 'text-green-900'
															: 'text-red-900'}"
													>
													...
													{#if caseItem.reviewNotes}
														<p
															class="mt-1 text-sm {caseItem.status === 'APPROVED'
																? 'text-green-700'
																: 'text-red-700'}"
														>
...
{#if showCaseModal && selectedCaseDetails}
	...
	>
		...
		>
			<div
				class="sticky top-0 flex items-center justify-between border-b border-border bg-surface p-6"
			>
			...
			<div class="p-6 space-y-6">
				<!-- Case Status -->
				<div
					class="rounded-xl {selectedCaseDetails.status === 'APPROVED'
						? 'bg-green-50 border-green-200'
						: selectedCaseDetails.status === 'REJECTED'
							? 'bg-red-50 border-red-200'
							: 'bg-yellow-50 border-yellow-200'} border p-4"
				>
					<div class="flex items-center justify-between">
						<div>
							<p class="text-sm font-medium text-surface-muted">Case Status</p>
							<p
								class="mt-1 text-2xl font-bold {selectedCaseDetails.status === 'APPROVED'
									? 'text-green-600'
									: selectedCaseDetails.status === 'REJECTED'
										? 'text-red-600'
										: 'text-yellow-600'}"
						...
						<div class="text-right">
							...
							<p class="mt-1 text-2xl font-bold text-surface-emphasis">
				...
				<div>
					...
					<div class="grid grid-cols-2 gap-4 rounded-xl bg-surface-soft p-4">
						<div>
							...
							<p class="font-semibold text-surface-emphasis">
								{selectedCaseDetails.patient?.name || 'N/A'}
							</p>
						</div>
						<div>
							<p class="text-sm text-surface-muted">Age</p>
							<p class="font-semibold text-surface-emphasis">
								{selectedCaseDetails.patient?.age || 'N/A'}
							</p>
						</div>
						<div>
							<p class="text-sm text-surface-muted">Gender</p>
							<p class="font-semibold text-surface-emphasis">
								{selectedCaseDetails.patient?.gender || 'N/A'}
							</p>
						</div>
						<div>
							<p class="text-sm text-surface-muted">Phone</p>
							<p class="font-semibold text-surface-emphasis">
								{selectedCaseDetails.patient?.phone || 'N/A'}
							</p>
						</div>
					</div>
				</div>
				...
				{#if selectedCaseDetails.chiefComplaint}
					<div>
						...
						<p class="rounded-xl bg-surface-soft p-4 text-surface-emphasis">
							{selectedCaseDetails.chiefComplaint}
						</p>
					</div>
				{/if}
				...
				{#if selectedCaseDetails.status === 'APPROVED' || selectedCaseDetails.status === 'REJECTED'}
					<div>
						...
						>
							<p
								class="font-semibold {selectedCaseDetails.status === 'APPROVED'
									? 'text-green-900'
									: 'text-red-900'}"
							>
							...
							{#if selectedCaseDetails.reviewNotes}
								<p
									class="mt-2 text-sm {selectedCaseDetails.status === 'APPROVED'
										? 'text-green-700'
										: 'text-red-700'}"
								>
									<strong>Feedback:</strong>
									{selectedCaseDetails.reviewNotes}
								</p>
							{/if}
						</div>
					</div>
